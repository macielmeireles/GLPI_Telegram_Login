...
if ($auth->login($login, $password, (isset($_REQUEST["noAUTO"]) ? $_REQUEST["noAUTO"] : false), $remember, $login_auth)) {
    shell_exec('/www/site/sendTelegram.sh "GLPI - Login Bem Sucessido: ' . $login . ' | IP de Origem: ' . $ip . '"');
    Auth::redirectIfAuthenticated();
} else {
    shell_exec('/www/site/sendTelegram.sh "GLPI - Erro de autenticação: ' . $login . ' | IP de Origem: ' . $ip . '"');
